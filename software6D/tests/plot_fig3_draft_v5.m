function plot_fig3_draft_v5()
%PLOT_FIG3_DRAFT_V5  Publication-size Figure 3. v5 = v4 cleanup:
%   - All four panels forced square (axis square).
%   - Panel labels A/B/C/D moved further outside (no more clash with
%     Panel A's top tick value or Panel B's y-label).
%   - Explicit, identical tick mark length on all four panels.
%   - Tighter whitespace between tiles.
%
...
set(fig, ...
    ...
    'DefaultColorbarTickLength', 0.018, ...
...
% Fewer ticks (3 per axis) to match the tick-density of C and D.
set(ax, 'XTick', [0 round(xyMax/2) xyMax], 'YTick', [0 round(xyMax/2) xyMax], ...
        'TickLength', [0.018 0.018], 'DataAspectRatio', [1 1 1], ...
...
set(ax, 'XTick', tickVals, 'YTick', tickVals, 'TickLength', [0.018 0.018], ...
...
outPath = fullfile(outDir, 'fig3_draft_v5.png');
...
% Use title() — it lives in the axes' outer margin (above the box),
% never inside the plot area, never on tick labels.
t = title(ax, letter, 'FontSize', 24, 'FontWeight', 'bold', ...
          'FontName', 'Calibri', 'HorizontalAlignment', 'left');
t.Units = 'normalized';
t.Position = [-0.22 1.02 0];  % 22% left of axes, 2% above top
end
