function plot_fig3_draft_v1()
%PLOT_FIG3_DRAFT_V1  Draft of Figure 3 (algorithm benchmarking).
%
%  Built from the 225 paired (3D vs 6D) Monte-Carlo samples in
%  tests/benchmark_output/mc/mc_results.csv. Four panels:
%
%    A. Paired linkAcc scatter (3D vs 6D), colored by NoiseXY
%    B. Paired nMerged scatter (3D vs 6D), colored by particle density
%    C. Per-metric "fraction of MC samples where 6D wins" with
%       bootstrap 95% CI and median delta annotation
%    D. Spearman driver heatmap: input parameters vs sign-aligned
%       delta for each metric. Identifies the regime where 6D matters.
%
%  Output: Manuscripts\6D Tracking Paper_REAL\Figure Outline\fig3_draft_v1.png
%
...
% debug: print sign-aligned medians and fractions before sort
fprintf('\nPanel C (pre-sort, sign-aligned where +Delta == 6D-better):\n');
for k = 1:nM
    fprintf('  %-22s  frac6Dbetter=%.3f  medDelta=%+.4g\n', ...
            metrics{k, 3}, fracs(k), medDelt(k));
end
...
yPos = (1:nM)';            % increasing, top-of-axis = best metric (via reverse YDir)
for k = 1:nM
    if fracs(k) >= 0.5
...
% annotate median delta to the right of each bar
for k = 1:nM
    text(ax, max(102, 100*ciHi(k) + 3), yPos(k), ...
        sprintf('  med \\Delta = %s', fmtDelta(medDelt(k), labels{k})), ...
...
% ---------------- title + save ----------------
title(tl, sprintf(['Figure 3 (draft v1). Orientation-aware 6D tracking: ' ...
...
outPath = fullfile(outDir, 'fig3_draft_v1.png');
...
if contains(label, 'accuracy') || contains(label, 'purity') || ...
   ...
   contains(label, 'alpha')
    s = sprintf('%+.2f pp', 100*v);   % percentage-point
...
%DIVERGINGCOLORMAP  Blue-white-red diverging map, n rows.
if nargin < 1, n = 64; end
half = floor(n/2);
t = linspace(0, 1, half)';
top    = [t, t, ones(half, 1)];        % deep blue -> white
bottom = [ones(half,1), 1-t, 1-t];     % white -> deep red
if mod(n, 2) == 1
